...
set versionstring = `${bindir}/yosys -V | cut -d' ' -f2  | cut -d'+' -f1`
...
if (( ${major} == 0 && ${minor} == 3 && ${revision} == 0 && ${subrevision} >= 514) || ( ${major} == 0 && ${minor} == 3 && ${revision} > 0 ) || ( ${major} == 0 && ${minor} > 3 ) ||  ( ${major} > 0) ) then
cat >> ${rootname}.ys << EOF
read_liberty -lib -ignore_miss_dir -setattr blackbox ${techdir}/${libertyfile}
EOF
